  ...
  while(next_statement(&letter, &value, line, &char_counter)) {
    ...
    switch(letter) {
	  ...
	  if (next_action == NEXT_ACTION_GO_HOME) {
		...
		// set the target and position to limit positon
		target[letter - 'X'] = Limit_Position;
		gc.position[letter - 'X'] = Limit_Position;
		plan_set_current_position(gc.position[X_AXIS],gc.position[Y_AXIS],gc.position[Z_AXIS]);
  
		// set target and position to zero
		target[letter - 'X'] = 0;
		gc.position[letter - 'X'] = 0;
		// move axis to zero
