...
uint8_t gc_execute_line(char *line)
{
  ...
  */
  ...
  #ifdef VARIABLE_SPINDLE
	// NNW
	if ((gc_state.spindle_speed != gc_block.values.s) || (SpindelSpeedParsed > 0)) // This also seems to be a bug because (SpindelSpeedParsed >= 0) is always true.
	{
	// TIMO: the second '=' within the if looks like a bug: was (gc_block.modal.motion = MOTION_MODE_LINEAR )
